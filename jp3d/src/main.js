...
import { UnrealBloomPass } from "three/addons/postprocessing/UnrealBloomPass.js";// Scene Setup
...
const bloomPulse = {
  idleTime: 2.5, // Time spent at 0.5 (seconds)
  spikeTime: 2.5, // Time to reach peak (seconds)
  fallTime: .5, // Time to return (seconds)
  minRadius: 0.5,
  maxRadius: 1,
  timer: 0, // Cycle timer
};
...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    gltf.scene.traverse((child) => {
      ...
      child.geometry.computeBoundingSphere(); // Helps with culling
      if (!/pointer/i.test(child.name)) { // Don't disable for pointer objects
      ...
      if (/pointer/i.test(child.name)) {
        ...
        child.renderOrder = 1;
        // Removed the forced red material assignment
      }
